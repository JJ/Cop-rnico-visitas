...
author: "JJ Merelo-Guervós and Elena Merelo-Molina"
date: "March 9th,2017"
output:
  pdf_document:
    keep_tex: true
...
been many studies analyzing a similar effect aroused by mentions in
...
Madrid, in which a video newscaster referred to a character in a float as
...
when a journalist and a politician discussed in a TV program about
...
other phenomenons of visits to the Wikipedia which have happened in a
similar time frame.
...
The rest of the paper is organized in the following way: first we present
...
related webpages is analyzed, along with the evolution in its
...
allows access to the visits of each and every one of the different
...
programs, is free software and hence can be downloaded from GitHub
...
datos <- data.frame( day=c(1:nrow(datos.2016),1:nrow(datos.2017)),
...
```{r imagen,echo=FALSE}
ggplot(datos,aes(x=day,y=visits,group=year,col=year))+geom_line()+theme_tufte()
```
...
```{r ratio,echo=FALSE}
ratio <- data.frame( day=c(1:nrow(datos.2016)),ratio=datos.2017$V1/datos.2016$V1)
ggplot(ratio,aes(x=day,y=ratio))+geom_line()+theme_tufte()
```
...
hc.datos <- data.frame( day=c(1:nrow(hc.2016),1:nrow(hc.2017)),
                    year=c(rep('2016',nrow(hc.2016)),rep('2017',nrow(hc.2017))),
                    visits=c(hc.2016$V1,hc.2017$V1))
ggplot(hc.datos,aes(x=day,y=visits,group=year,col=year))+geom_line()+theme_tufte()
```
The number of visits moves in a different scale and thus there are
more fluctuations along the day. However, like in the previous case,
...
hc.ratios <- data.frame( day=c(1:nrow(ratio.hc.2016),1:nrow(ratio.hc.2017)),
                    year=c(rep('2016',nrow(ratio.hc.2016)),rep('2017',nrow(ratio.hc.2017))),
                    visits=c(ratio.hc.2016$V1,ratio.hc.2017$V1))
ggplot(hc.ratios,aes(x=day,y=visits,group=year,col=year))+geom_line()+theme_tufte()
```
...
galileo.datos <- data.frame( day=c(1:nrow(galileo.2016),1:nrow(galileo.2017)),
                    year=c(rep('2016',nrow(galileo.2016)),rep('2017',nrow(galileo.2017))),
                    visits=c(galileo.2016$V1,galileo.2017$V1))
ggplot(galileo.datos,aes(x=day,y=visits,group=year,col=year))+geom_line()+theme_tufte()
```
...
Galileo´s is linked to Copérnico´s, and as there are not changes in the
...
meryl.datos <- data.frame( day=c(1:nrow(meryl.2016),1:nrow(meryl.2017)),
                    year=c(rep('2016',nrow(meryl.2016)),rep('2017',nrow(meryl.2017))),
                    visits=c(meryl.2016$V1,meryl.2017$V1))
ggplot(meryl.datos,aes(x=day,y=visits,group=year,col=year))+geom_point()+geom_smooth(span=0.25)+theme_tufte()

john.datos <- data.frame( day=c(1:nrow(john.2016),1:nrow(john.2017)),
                    year=c(rep('2016',nrow(john.2016)),rep('2017',nrow(john.2017))),
                    visits=c(john.2016$V1,john.2017$V1))
ggplot(john.datos,aes(x=day,y=visits,group=year,col=year))+geom_point()+geom_smooth(span=0.25)+theme_tufte()
```

The number of visits reached by John Hurt's Wikipedia page is almost 3 times as high as the one reached by Meryl Streep. The mechanism at work here must go in the opposite direction of fame: John Hurt was not probably known by name, so people looked him up much more than Meryl Streep, who is a household name and whose movies are shown continuously, even in Spanish TV channels. But even with the different scales, the *relaxation time* hovers around 7 days, a few more in the case of Meryl Streep (left). Besides, when the visits plateau the *new normal* is slightly superior to the visits achieved in the previous year or before the incident that initiated the sudden fame. Let us see this in the second case, the John Hurt page.
...
vaughn.datos <- data.frame( day=c(1:nrow(vaughn.2016),1:nrow(vaughn.2017)),
                    year=c(rep('2015',nrow(vaughn.2016)),rep('2016',nrow(vaughn.2017))),
                    visits=c(vaughn.2016$V1,vaughn.2017$V1))
ggplot(vaughn.datos,aes(x=day,y=visits,group=year,col=year))+geom_point()+geom_smooth(span=0.25)+theme_tufte()

carrie.2016 <- read.table('datos/Carrie-Fisher-2016.dat')
carrie.2017 <- read.table('datos/Carrie-Fisher-2017.dat')
carrie.datos <- data.frame( day=c(1:nrow(carrie.2016),1:nrow(carrie.2017)),
                    year=c(rep('2016',nrow(carrie.2016)),rep('2017',nrow(carrie.2017))),
                    visits=c(carrie.2016$V1,carrie.2017$V1))
ggplot(carrie.datos,aes(x=day,y=visits,group=year,col=year))+geom_point()+geom_smooth(span=0.25)+theme_tufte()

# George Michael
george.2016 <- read.table('datos/George-Michael-2016.dat')
george.2017 <- read.table('datos/George-Michael-2017.dat')
george.datos <- data.frame( day=c(1:nrow(george.2016),1:nrow(george.2017)),
                    year=c(rep('2016',nrow(george.2016)),rep('2017',nrow(george.2017))),
                    visits=c(george.2016$V1,george.2017$V1))
ggplot(george.datos,aes(x=day,y=visits,group=year,col=year))+geom_point()+geom_smooth(span=0.25)+theme_tufte()
```
...
Eventually, all visit graphs resemble a "witch´s hat", with a noteworthy peak and a more gradual decrease in the number of visits in approximately a week´s time, although usually a bit longer. We will examine this findings in the next section, where we present our conclusions. 
...
All files, data and scripts needed to generate this paper are available at [the GitHub repository for this paper](http://github.com/JJ/Cop-rnico-visitas) and can be used with a free license. If you use in any scientific publication we are grateful for referencing this paper or the other papers working on the same data included in the bibliography. 
